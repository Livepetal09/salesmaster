...
if($status==0){header('location: login.php'); }
if(!isset($_SESSION['user'])){
    header('location: login.php'); exit;
}

if(!isset($_SESSION["salesid"])) { $_SESSION["salesid"] = rand();     }
...
if(isset($_GET['edit'])){
  ...
  if($sql){Alert('Successfully Added to cart');
...
if($total==0){header('location: ?'); }
  ...
  if($sql){Alert('Successfully Submitted');;
...
<?php include('nav.php') ?>
    <div class="row">
        <div class="col-md-12">
            <h2 style=""><?php if(isset($_COOKIE['biztitle'])){ echo strtoupper($_COOKIE['biztitle']); } ?><br>Point of Sale</h2>
        </div>
    </div>
...
<button type="submit" class="btn btn-primary btn-block" name="AddItem">Add Item to Cart</button>
    ...
    <div class="card-body">
<table class="table">
<tr><th>Qty</th><th>Item</th><th>Price</th><th>Amount</th><th>Action</th></tr>
...
<input type="text" class="form-control" name="customer" required><br>
...
<?php $i=1; $sql = $db->query("SELECT * FROM sales WHERE bid='$bid' AND user='$user' ORDER BY sn DESC LIMIT 20");
